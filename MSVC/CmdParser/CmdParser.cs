...
namespace CmdParser
{
    ...
    public sealed class Parser
    {
            ...
            foreach (Argument argument in this.arguments)
            {
                ...
                if (argument.ExplicitShortName)
                {
                    if (argument.ShortName != null && argument.ShortName.Length > 0 &&
                        argument.ShortName.ToLower() != argument.LongName.ToLower())
                    {
        ...
        /// <summary>
        /// Optionally breaks a single option argument string into multiple option argument
        /// strings if appropriate.
        /// </summary>
        /// <param name="arg"></param>
        /// <param name="optionArgument"></param>
        /// <returns></returns>
        private string[] GetOptionArgumentList(Argument arg, string optionArgument)
        {
            // Support splitting any value type (like an enum or number) into a list of optional
            // arguments, except if it is a string.
            if (arg.ValueType.IsSubclassOf(typeof(System.ValueType)) && arg.ValueType != typeof(char) && optionArgument != null)
            {
                return optionArgument.Split(':');
            }
            return new string[] { optionArgument };
        }
                                ...
                                else
                                {
                                    if ((optionArgument == null || optionArgument.Length == 0) && i < args.Length - 1)
                                    {
                                        ...
                                        if (hadErrorNow)
                                        {
                                            ...
                                            hadErrorNow = false;
                                            foreach (string opt in GetOptionArgumentList(arg, optionArgument))
                                            {
                                                hadErrorNow |= !arg.SetValue(opt, destination);
                                            }
                                        }
                                    ...
                                    else
                                    {
                                        foreach (string opt in GetOptionArgumentList(arg, optionArgument))
                                        {
                                            hadError |= !arg.SetValue(opt, destination);
                                        }
                                    }
                                }
                                break;
        ...
        private class Argument
        {
            ...
            private void AppendValue(StringBuilder builder, object value)
            {
                if (value is string || value is int || value is uint || value is float || value is double
                    || value is long || value is ulong || value is short || value is ushort || value is char || value.GetType().IsEnum)
                {
    ...
    /*****************Below added by qiangwu*******************************/
    public class CmdOptions
    {
        public void Print()
        {
            Type t = this.GetType();
            FieldInfo[] finfs = t.GetFields();
            for (int i = 0; i < finfs.Length; ++i)
                Console.WriteLine(finfs[i].Name + " = " + finfs[i].GetValue(this));
        }
    }
}
