        ...
        for (int k = 0; k < this->blob_bottom_->height(); ++k) {
            for (int l = 0; l < this->blob_bottom_->width(); ++l) {
                ...
                for (int g = 0; g < groups_; ++g) {
                    ...
                    for (int j = 0; j < size; ++j) {
                        EXPECT_GE(this->blob_top_->data_at(i, offset + j, k, l) + 1e-4,
                                  exp(this->blob_bottom_->data_at(i, offset + j, k, l)) / scale)
                            << "debug: " << i << " " << offset + j;
                        EXPECT_LE(this->blob_top_->data_at(i, offset + j, k, l) - 1e-4,
                                  exp(this->blob_bottom_->data_at(i, offset + j, k, l)) / scale)
                            << "debug: " << i << " " << offset + j;
                    }
