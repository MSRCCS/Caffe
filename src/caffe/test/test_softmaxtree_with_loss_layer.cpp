...
TYPED_TEST(SoftmaxTreeWithLossLayerTest, TestForward) {
    ...
    for (int i = 0; i < this->blob_bottom_data_->num(); ++i) {
        for (int k = 0; k < this->blob_bottom_data_->height(); ++k) {
            for (int l = 0; l < this->blob_bottom_data_->width(); ++l) {
                int label_value = static_cast<int>(this->blob_bottom_label_->data_at(i, 0, k, l));
                EXPECT_GE(label_value, 0);
                EXPECT_LT(label_value, this->blob_top_prob_->shape(1));
                while (label_value >= 0) {
                    loss -= log(std::max(this->blob_top_prob_->data_at(i, label_value, k, l), Dtype(FLT_MIN)));
                    ++count;
    ...
    layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
    layer->Forward(this->blob_bottom_vec_, this->blob_top_vec_);
    EXPECT_NEAR(loss / count, this->blob_top_loss_->cpu_data()[0], 1e-3);
}
...
TYPED_TEST(SoftmaxTreeWithLossLayerTest, TestForwardIgnoreLabel) {
    ...
    Blob<Dtype> label_orig(this->blob_bottom_label_->shape());
    label_orig.CopyFrom(*this->blob_bottom_label_);
    ...
    for (int label = 0; label < 11; ++label) {
        ...
        // Revert back
        this->blob_bottom_label_->CopyFrom(label_orig);
    }
...
TYPED_TEST(SoftmaxTreeWithLossLayerTest, TestForwardWithObjectness) {
    ...
    int count = 0;
    for (int i = 0; i < this->blob_bottom_data_->num(); ++i) {
        const int label_value_orig = static_cast<int>(this->blob_bottom_label_->data_at(i, 0, 0, 0));
        EXPECT_GE(label_value_orig, 0);
        EXPECT_LT(label_value_orig, this->blob_top_prob_->shape(1));
        double max_prob = -1;
        int max_k = 0;
        int max_l = 0;
        ...
        for (int k = 0; k < this->blob_bottom_data_->height(); ++k) {
            for (int l = 0; l < this->blob_bottom_data_->width(); ++l) {
                double p = this->blob_bottom_objectness_->data_at(i, 0, k, l);
                int label_value = label_value_orig;
                while (label_value >= 0) {
                    p *= this->blob_top_prob_->data_at(i, label_value, k, l);
                    label_value = this->parent_[label_value];
                }
                if (p > max_prob) {
                    max_prob = p;
...
TYPED_TEST(SoftmaxTreeWithLossLayerTest, TestForwardWithObjectnessPerBatchLabels) {
    ...
    int count = 0;
    for (int i = 0; i < this->blob_bottom_data_->num(); ++i) {
        const int label_value_orig = static_cast<int>(this->blob_bottom_label_perbatch_->data_at({ i, 0 }));
        EXPECT_GE(label_value_orig, 0);
        EXPECT_LT(label_value_orig, this->blob_top_prob_->shape(1));
        double max_prob = -1;
        int max_k = 0;
